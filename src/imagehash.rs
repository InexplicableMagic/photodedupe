...
impl ImageHashAV {
	...
	pub const DEFAULT_ALG_FLIP_THRESHOLD : u64 = 10000; //Number of files at which we flip to the less accurate but faster algorithm
		
	pub fn new(fpath : &str) -> Result<ImageHashAV,MyImageError> {
		let mut object = ImageHashAV { dupe_group: 0, grey_hash: 0, low_res: [0;192], width: 0, height: 0, num_pixels: 0, std_dev: 0f32, file_size: 0, fpath: "".to_string() };
		let result = object.calc_image_hash( &fpath )?;
	...
	pub fn calc_image_hash(&mut self, fpath: &str ) -> Result<(), MyImageError> {
		   
		let img = load_image_from_file( fpath )?;
		
		let (width, height) = img.dimensions();
		if width < 16 || height < 16 {
			return Err( MyImageError::ImageTooSmall(format!("Warning: Image too small to deduplicate: {}", fpath).to_string()) );
		}
		...
		match fs::metadata(fpath) {
			...
			Err(_)=> {
				return Err(MyImageError::FileError(format!("Error: Failed to get size of: {}", fpath).to_string()));
			}
		}
		...
		if width != 8 || height != 8 {
			return Err( MyImageError::DecodeFail(format!("Error: Failed to resize image correctly: {}", fpath).to_string()) );
		}
		...
		//eprintln!("grayHash: {:x} {}", hash_val, fpath);
...
mod tests {
    ...
    fn test_image_duplicates() {
			...
			for i in 0..(image_paths.len()/3) {
				let mut result = ImageHashAV::new( &image_paths[i*3] ).unwrap();
				...
				assert!( ImageHashAV::calc_hamming_distance(result.dupe_group, dupe1.dupe_group) >= 63, "First duplicate grey hash matches" );
				assert!( ImageHashAV::calc_hamming_distance(result.dupe_group, dupe2.dupe_group) >= 63, "Second duplicate grey hash matches" );
    ...
    fn test_image_uniques() {
			...
			for i in 0..image_hashes.len() {			
				for j in (i+1)..image_hashes.len() {
					...
					assert!( (ImageHashAV::calc_hamming_distance(image_hashes[j].dupe_group, image_hashes[i].dupe_group) <= 63) || 
